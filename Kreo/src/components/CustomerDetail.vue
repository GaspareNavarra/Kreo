<template>
  <div>
    <div class="welcome-message col-sm-12 mb-4">
      {{ customer.name + " " + customer.surname }}
    </div>
    <div class="timeline-container">
      <Timeline :value="entrances">
        <template #marker>
          <i class="pi pi-check-circle" style="font-weight: bolder"></i>
          <!-- style="color: #2bb163" -->
        </template>
        <template #content="slotProps">
          <Skeleton
            v-if="entrances?.entrances"
            height="10rem"
            class="card-spacer"
          ></Skeleton>
          <Card class="card-spacer" v-if="entrances.length > 0">
            <template #title>{{
              "Ingresso avvenuto il " + formatDate(slotProps.item.data_ingresso)
            }}</template>
            <template #subtitle>
              <pre style="font-family: 'Poppins'">{{
                "Note: \n" + slotProps.item.note
              }}</pre>
            </template>
            <template #content>
...
export default {
  inject: ["classSelector", "decryptTreatments", "viewMessage"],
  data() {
    return {
  ...
  methods: {
    getEntrances() {
      ...
      axios
        .post(window.BASE_URL_API_XANO + "/ingressi-customer", body, config)
        .then((response) => {
    ...
    getTretments(ingressi) {
      ...
      axios
        .get(window.BASE_URL_API_XANO + "/treatments", config)
        .then((response) => {
          let data = response.data;
          Object.values(ingressi).forEach((ingresso) => {
            ingresso.trattamento = this.decryptTreatments(
              data,
